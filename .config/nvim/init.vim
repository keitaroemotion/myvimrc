nnoremap <silent> <C-t> <cmd>below new<CR><cmd>terminal<CR>
nnoremap <silent> T <cmd>belowright new<CR><cmd>terminal<CR>
autocmd TermOpen * :startinsert
autocmd TermOpen * setlocal norelativenumber
autocmd TermOpen * setlocal nonumber

"nnoremap P :vimgrep func %<CR> :cwindow<CR>

:command! -nargs=? Memo :call Memo()
function Memo()
   exe 'edit /Users/kei.sugano/sugamemo/mms/memo'
endfunction

:set encoding=utf-8

:command! -nargs=? Func :call Func()
function Func()
   exe 'vimgrep func %'
   exe 'cwindow'
endfunction
...
command! -nargs=0 Vimrc :call Vimrc()
function! Vimrc()
...
function! Run(...)
    ...
    let file   = @%
    let result = system("chmod +x " . file)
    let result = system("" . file . " " . postfix)
    echo result
endfunction
...
function! Look(...)
    ...
    execute ':! open' .  ' "https://www.google.com/search?q=' . escape(expand('<cword>') . alphaquery, ' "') . '"'
endfunction
...
function! Blame()
    let file   = @%
    let linenum = line(".")
    let result = system("git blame " . "-L " . linenum . "," . linenum . " " . file)
    echo result
endfunction
...
let s:dein_base = '/Users/kei.sugano/.cache/dein'

" Set Dein source path (required)
let s:dein_src = '/Users/kei.sugano/.cache/dein/repos/github.com/Shougo/dein.vim'
